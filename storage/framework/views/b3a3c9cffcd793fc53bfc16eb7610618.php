<?php $__env->startSection('title', 'Generated Diplomas & Certificates'); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['class' => 'size-7 text-amber-600']); ?>
...
<?php endif; ?>
        ...
        <div class="flex flex-wrap items-center gap-4 mb-6">
            <form class="max-w-md w-full sm:w-80" role="search" method="GET" action="<?php echo e(route('diploma.records')); ?>">
...
<?php endif; ?>
            ...
            <div class="flex items-center gap-3 ml-auto">
                <form method="GET" action="<?php echo e(route('diploma.records')); ?>" id="limitForm">
                ...
                <thead class="text-xs uppercase font-semibold text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-neutral-800 border-b border-slate-200 dark:border-neutral-700">
                    <tr>
                        <th scope="col" class="px-4 py-3.5 w-16">
                            <a href="<?php echo e(route('diploma.records', array_merge($queryParams, ['sort' => 'id', 'direction' => $nextDirection('id')]))); ?>" class="flex items-center gap-1">
                                ID
...
<?php $attributes = $attributes->except(\BladeUI\Icons\Components\Svg::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['class' => 'size-3 text-amber-600']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginal643fe1b47aec0b76658e1a0200b34b2c)): ?>
<?php $attributes = $__attributesOriginal643fe1b47aec0b76658e1a0200b34b2c; ?>
...
<?php endif; ?>
                ...
                <tbody class="text-sm divide-y divide-slate-200 dark:divide-neutral-700 bg-white dark:bg-neutral-900">
                    <?php $__empty_1 = true; $__currentLoopData = $diplomas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $diploma): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/70 dark:hover:bg-neutral-800/40 transition-colors">
                            <td class="px-4 py-3.5 font-semibold text-slate-500 text-xs">
                                #<?php echo e($diploma->id); ?>

                            </td>
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-3">
                                    <img src="<?php echo e($diploma->photo ?: asset('diploma-studio/assets/student-sample.jpg')); ?>"
                                         alt="<?php echo e($diploma->student_name); ?>"
                                         class="w-9 h-11 object-cover rounded border border-slate-200 dark:border-neutral-700 shrink-0 shadow-sm" />
                                    <div>
                                        <div class="font-semibold text-slate-900 dark:text-slate-100">
                                            <?php echo e($diploma->gender); ?> <?php echo e($diploma->student_name); ?>

                                        </div>
                                        <?php if($diploma->father_name): ?>
                                            <div class="text-xs text-slate-500">Father: <?php echo e($diploma->father_name); ?></div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3.5 font-medium text-slate-800 dark:text-slate-200">
                                <span class="px-2 py-0.5 rounded bg-amber-50 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300 font-mono text-xs font-semibold">
                                    <?php echo e($diploma->serial_no ?: '-'); ?>

                                </span>
                            </td>
                            <td class="px-4 py-3.5 font-mono text-xs text-slate-600 dark:text-slate-400">
                                <?php echo e($diploma->enrollment_no ?: '-'); ?>

                            </td>
                            <td class="px-4 py-3.5 text-slate-800 dark:text-slate-200">
                                <div class="font-medium text-xs sm:text-sm line-clamp-1" title="<?php echo e($diploma->course_name); ?>"><?php echo e($diploma->course_name ?: '-'); ?></div>
                                <div class="text-xs text-slate-500"><?php echo e($diploma->location ?: 'Palghar (Maharashtra)'); ?></div>
                            </td>
                            <td class="px-4 py-3.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400 whitespace-nowrap">
                                <?php echo \App\Http\Controllers\DiplomaController::formatDivision($diploma->division); ?>

                            </td>
                            <td class="px-4 py-3.5 text-xs text-slate-500 whitespace-nowrap">
                                <?php echo e($diploma->cert_date ? date('M d, Y', strtotime($diploma->cert_date)) : '-'); ?>

                            </td>
                            <td class="px-4 py-3.5 text-right whitespace-nowrap">
                                <div class="flex items-center justify-end gap-2">
                                    <!-- Print Standalone -->
                                    <a href="<?php echo e(route('diploma.print', ['id' => $diploma->id])); ?>" target="_blank"
...
<?php endif; ?>
                                    ...
                                    <!-- Edit in Studio -->
                                    <a href="<?php echo e(route('diploma.index', ['id' => $diploma->id])); ?>"
...
<?php endif; ?>
                                    ...
                                    <!-- Delete -->
                                    <form action="<?php echo e(route('diploma.destroy', $diploma->id)); ?>" method="POST" onsubmit="return confirm('Are you sure you want to delete this Diploma record?');" class="inline">
                                        <?php echo csrf_field(); ?>
...
<?php endif; ?>
                                ...
                                <a href="<?php echo e(route('diploma.index')); ?>" class="inline-flex items-center gap-1.5 mt-4 px-3.5 py-2 text-xs font-semibold rounded-md bg-amber-600 text-white hover:bg-amber-700">
                                    <?php if (isset($component)) { $__componentOriginal643fe1b47aec0b76658e1a0200b34b2c = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal643fe1b47aec0b76658e1a0200b34b2c = $attributes; } ?>
<?php $component = BladeUI\Icons\Components\Svg::resolve([] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('lucide-plus'); ?>
...
<?php endif; ?> Open Studio
        ...
        <!-- Pagination -->
        <?php if($diplomas->hasPages()): ?>
            <div class="mt-6">
                <?php echo e($diplomas->appends(request()->query())->links()); ?>
...
<?php echo $__env->make('layouts.backend', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/willpowe/domains/apeirojobs.com/resources/views/backend/diploma/records.blade.php ENDPATH**/ ?>