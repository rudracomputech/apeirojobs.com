...
<body>
  ...
  <header class="app-header">
    ...
    <div class="header-actions">
      ...
      @endif
      <a href="{{ route('migration.records') }}" class="btn btn-secondary" style="background: #2563eb; color: #fff;">
  ...
  <div class="studio-layout">
    ...
    <aside class="sidebar-panel">
      ...
      <div class="sidebar-scroll">
        <form id="certForm" method="POST" action="{{ route('migration.print') }}" target="_blank">
          @csrf
            ...
            <div class="form-card-body">
              ...
              <div class="form-group">
                ...
                <input type="text" id="input_watermarkText" name="watermarkText" class="form-input"
                  value="{{ $watermarkSource }}" placeholder="Auto-syncs with Council Main Name">
              </div>
          ...
          <div class="form-card">
            ...
            <div class="form-card-body">
              ...
              <div class="form-group">
                ...
                <textarea id="input_clause2" name="clause2" class="form-textarea">{{ $data['clause2'] }}</textarea>
              </div>
            </div>
          </div>
            ...
            <div class="cert-inner">
                  ...
                  <defs>
                    <pattern id="watermarkTextPattern" width="750" height="24" patternUnits="userSpaceOnUse">
                      <text id="cert_watermarkPatternText" x="0" y="16"
                        ...
                        letter-spacing="1.5">{{ $watermarkRepeat }}</text>
                    </pattern>
                  </defs>
              ...
              <div class="cert-header">
                ...
                <div class="cert-top-bar">
                  <div class="cert-serial-no">
                    Serial No.: <span id="cert_serialNo">{{ $data['serialNo'] }}</span>
                  </div>
                </div>

                <!-- Council Logo Emblem -->
                <div class="cert-logo-container">
                  <img id="cert_logo" src="{{ $data['logoSrc'] }}" class="cert-logo-img" alt="Council Emblem" />
                </div>
                ...
                <div class="cartouche-frame">
                  ...
                  <div class="cartouche-pill-outer">
                    <div class="cartouche-pill">
                      <span class="cartouche-text" id="cert_certTitle">{{ $data['certTitle'] }}</span>
                    </div>
                  </div>
              ...
              <div class="cert-statement-body">
                ...
                <div class="statement-row row-1">
                  <span class="static-prompt" id="cert_studentPrefix">{{ $data['studentPrefix'] }}</span>
                  <div class="fill-slot slot-student">
                    <span class="fill-value" id="cert_studentName">{{ $data['studentName'] }}</span>
                  </div>
                  <span class="static-prompt" id="cert_sdPrefix">{{ $data['sdPrefix'] }}</span>
                  <div class="fill-slot slot-parent">
                    <span class="fill-value" id="cert_parentName">{{ $data['parentName'] }}</span>
                  </div>
                </div>

                <!-- LINE 2 -->
                <div class="statement-row row-2">
                  <span class="static-prompt" id="cert_passedText">{{ $data['passedText'] }}</span>
                  <div class="fill-slot slot-course">
                    <span class="fill-value" id="cert_courseName">{{ $data['courseName'] }}</span>
                  </div>
                  <span class="static-prompt" id="cert_fromText">{{ $data['fromText'] }}</span>
                </div>

                <!-- LINE 3 -->
                <div class="statement-row row-3">
                  <div class="fill-slot slot-inst">
                    <span class="fill-value" id="cert_instName">{{ $data['instName'] }}</span>
                  </div>
                  <span class="static-prompt" id="cert_inTheYearText">{{ $data['inTheYearText'] }}</span>
                  <div class="fill-slot slot-year">
                    <span class="fill-value" id="cert_passYear">{{ $data['passYear'] }}</span>
                  </div>
                  <span class="static-prompt" id="cert_bearingText">{{ $data['bearingText'] }}</span>
                </div>

                <!-- LINE 4 -->
                <div class="statement-row row-4">
                  <span class="static-prompt" id="cert_enrollmentLabel">{{ $data['enrollmentLabel'] }}</span>
                  <div class="fill-slot slot-enroll">
                    <span class="fill-value" id="cert_enrollmentNo">{{ $data['enrollmentNo'] }}</span>
                  </div>
                  <span class="static-prompt prompt-clause-1" id="cert_clause1">{{ $data['clause1'] }}</span>
                </div>

                <!-- LINE 5 -->
                <div class="statement-row row-5">
                  <span class="prompt-clause-2" id="cert_clause2">{{ $data['clause2'] }}</span>
                </div>
              ...
              <div class="cert-footer">
                ...
                <div class="footer-left">
                  <div class="footer-place" id="cert_placeName">{{ $data['placeName'] }}</div>
                  <div class="footer-date">
                    Dated : <span id="cert_certDate">{{ $data['certDate'] }}</span>
                  </div>
                </div>
                ...
                <div class="footer-right">
                  <div class="stamp-signature-box">
                    <img id="cert_stamp" src="{{ $data['stampSrc'] }}" class="official-stamp-img"
  ...
  <script>
    window.migrationAssetBase = "{{ asset('migration-studio/assets') }}/";
    window.migrationPrintUrl = "{{ route('migration.print') }}";
    window.migrationSaveUrl  = "{{ route('migration.save') }}";
  ...
  <script src="{{ asset('migration-studio/script.js') }}?v={{ time() }}"></script>
</body>
