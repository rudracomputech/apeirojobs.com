...
<div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-8">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 dark:text-slate-50">Installments</h2>
            <p class="text-sm text-slate-600 dark:text-slate-400">Manage invoice installments and payment schedules.</p>
        </div>
                ...
                <option value="all"{{ request('limit') == 'all' ? ' selected' : '' }}>All </option>
                   ...
                   <button type="button" id="filterBtn"
                      ...
                      <x-lucide-funnel class="size-4" />
                       Filter</button>
        <a href="{{ route('installments.create') }}" class="inline-flex items-center rounded-md bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">New Installment</a>
    </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-slate-200 text-sm text-left text-slate-700">
            ...
            <tbody class="divide-y divide-slate-200 bg-white">
                @forelse ($installments as $installment)
                    <tr>
                        <td class="px-4 py-3">{{ $installment->id }}</td>
                        <td class="px-4 py-3">{{ $installment->invoice->invoice_no ?? '—' }}</td>
                        <td class="px-4 py-3">{{ $installment->student->name ?? $installment->student->first_name ?? '—' }}</td>
                        <td class="px-4 py-3">{{ number_format($installment->amount, 2) }}</td>
                        <td class="px-4 py-3">{{ number_format($installment->paid_amount ?? 0, 2) }}</td>
                        <td class="px-4 py-3">{{ $installment->due_date?->format('Y-m-d') ?? '—' }}</td>
                        <td class="px-4 py-3">{{ ucfirst($installment->status) }}</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('installments.show', $installment->id) }}" class="text-slate-600 hover:text-slate-900">View</a>
                                <a href="{{ route('installments.edit', $installment->id) }}" class="text-blue-600 hover:text-blue-800">Edit</a>
                                <form action="{{ route('installments.destroy', $installment->id) }}" method="POST" onsubmit="return confirm('Delete this installment?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-4 py-6 text-center text-slate-500">No installments found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="offcanvas-right" class="overflow-y-scroll fixed top-0 right-0 z-50 h-full w-100 bg-white shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out p-6 flex flex-col justify-between">
    <div>
        ...
        <div class="space-y-4 text-gray-600">
            <form id="filter-form" method="GET" action="{{ route('installments.index') }}" class="flex flex-col space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="search" name="search" placeholder="Search installments..." value="{{ request('search') }}" class="w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
            </form>
        </div>
        ...
        <div class="border-t pt-4 flex gap-2">
            ...
            <a href="{{ route('installments.index') }}" class="flex-1 px-4 py-2 text-center text-slate-900 text-sm font-medium rounded-md border border-gray-300 hover:bg-gray-50">
                Reset
            </a>
        </div>
    </div>
