...
class User extends Authenticatable implements HasMedia, MustVerifyEmail
{
    ...
    public function hasRole($roles, ?string $guard = null): bool
    {
        ...
        if (is_string($roles)) {
            $roleTarget = strtolower($roles);
            return $guard
                ? $userRoles->where('guard_name', $guard)->contains(fn($r) => strtolower($r->name) === $roleTarget)
                : $userRoles->contains(fn($r) => strtolower($r->name) === $roleTarget);
        }
